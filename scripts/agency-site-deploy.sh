...
configure_public_domain() {
  ...
  local default_url default_portal_url public_url portal_url host session_cookie
  default_url="${PUBLIC_APP_URL:-https://gnd.520.ie}"
  default_portal_url="${PUBLIC_PORTAL_URL:-https://house.520.ie}"

  printf '请输入中介网站公网 URL（直接 Enter 使用 %s）：' "$default_url"
  read -r public_url
  public_url="$(normalize_public_url "${public_url:-$default_url}")"

  printf '请输入主站公网 URL，用于翻译网关（直接 Enter 使用 %s）：' "$default_portal_url"
  read -r portal_url
  portal_url="$(normalize_public_url "${portal_url:-$default_portal_url}")"

  host="$(printf '%s' "$public_url" | sed -E 's#^https?://([^/:]+).*#\1#')"
  session_cookie="$(printf '%s_session' "$host" | tr '.-' '__')"

  set_env_value "APP_ENV" "production"
  set_env_value "APP_DEBUG" "false"
  set_env_value "APP_URL" "$public_url"
  set_env_value "ASSET_URL" "$public_url"
  set_env_value "LIVEWIRE_ASSET_URL" "$public_url/vendor/livewire"
  set_env_value "TRANSLATION_GATEWAY_URL" "$portal_url/api/internal/translations/property"
  set_env_value "SESSION_COOKIE" "$session_cookie"
  set_env_value "SESSION_SECURE_COOKIE" "true"
  set_env_value "SESSION_SAME_SITE" "lax"

  rm -f .env.bak

  run_if_available "清理 Laravel URL/配置缓存" "$PHP_BIN" artisan optimize:clear
  run_if_available "重新生成生产缓存" "$PHP_BIN" artisan optimize

  info "中介网站公网域名已设置为：$public_url"
  info "翻译网关已设置为：$portal_url/api/internal/translations/property"
}
