...
configure_public_domain() {
  ...
  local default_url public_url host session_cookie
  default_url="${PUBLIC_APP_URL:-https://house.520.ie}"

  printf '请输入主网站公网 URL（直接 Enter 使用 %s）：' "$default_url"
  read -r public_url
  ...
  info "主网站公网域名已设置为：$public_url"
  info "页面资源和 Livewire 将使用公网 HTTPS，不再引用 localhost/127.0.0.1。"
}

require_app_dir() {
  [ -d "$APP_DIR" ] || fail "找不到主网站目录：$APP_DIR"
  [ -f "$APP_DIR/artisan" ] || fail "主网站目录中找不到 artisan：$APP_DIR"
}
...
create_backup() {
  ...
  info "备份完成：$output"
  info "这个包包含主网站源码、.env、database、storage/app 等业务数据；不包含 vendor/node_modules/build/cache/logs。"
}
